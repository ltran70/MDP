...
public class PolicyIteration extends DynamicProgramming implements Planner {
	...
	public GreedyQPolicy planFromState(State initialState) {
        ...
        if (delta <= this.maxPIDelta) {
            DPrint.cl(this.debugCode, "** Converged ** Total policy iterations: " + iterations + ". Delta: " + delta);
        } else {
            DPrint.cl(this.debugCode, "Total policy iterations: " + iterations + ". Delta: " + delta);
        }
	...
	protected double evaluatePolicy(){
		...
		for(i = 0; i < this.maxIterations; i++){
			...
			for(HashableState sh : states){
				...
				v = this.value(sh);
				maxQ = this.performFixedPolicyBellmanUpdateOn(sh, this.evaluativePolicy);
