	...
	public void runVI(){
		...
		for(i = 0; i < this.maxIterations; i++){
			...
			if(delta < this.maxDelta){
                DPrint.cl(this.debugCode, " ** Converged ** at " + i +" !!! Delta for the current run is " + delta);
                // for (Map.Entry<HashableState, Double> entry : this.valueFunction.entrySet()) {
                //     System.out.println("State: (" + entry.getKey().s().get("agent:x") +", "+ entry.getKey().s().get("agent:y") + "): " + entry.getValue());
                // }
		...
		DPrint.cl(this.debugCode, "Passes: " + i + ". Delta is " + delta);
